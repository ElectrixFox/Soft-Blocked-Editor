...
void _ExpandByOne(void **arr, const unsigned int size, unsigned int elesize) { ExpandArray(arr, size, size + 1, elesize); }
...
_ExpandByOne((void**)ta->action, n, sizeof(ui_act_fun));
_ExpandByOne((void**)ta->ui_id, n, sizeof(unsigned int));
...
if(index != size - 1)   // if the index is the last thing just decrease the size
    {
...
_ExpandByOne((void**)&ui->ui_id, n, sizeof(unsigned int));
_ExpandByOne((void**)&ui->trsid, n, sizeof(unsigned int));
_ExpandByOne((void**)&ui->data, n, datsize);
...
if(rendinf.rinf == (GUI_RENDER_INFO)NULL)
    ...
    ind = CreateBasicSquare(*rp, pos, scale, {1.0f, 0.0f, 0.0f, 1.0f});  // creates the square
    }
